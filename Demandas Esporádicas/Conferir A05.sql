...
CASE WHEN BO.codigo_municipio in (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
    ...
    WHERE
        1 = 1
        AND ENV.data_hora_fato >= '2024-01-01'
...
--        AND ENV.id_tipo_prisao_apreensao IN (1,2,3,4,6,7)-- FLAGRANTES(REDS E TCO),MANDADO, RECAPTURA, OUTRAS PRISOES
        AND ENV.numero_ocorrencia IN (
            SELECT
                bo.numero_ocorrencia 
            FROM
                db_bisp_reds_reporting.tb_ocorrencia BO
            WHERE
                1 = 1
                AND bo.data_hora_fato >= '2024-01-01'
                and BO.natureza_codigo  = 'A05000'
