/*---------------------------------------------------------------------------------------------------------------------------
 * Teste 01 - Preditivo
 * VERSAO FINAL: Verificar se os autores de homicídios possuem quantas passagens pretéritas por tráfico.
 ---------------------------------------------------------------------------------------------------------------------------*/
...
PASSAGENS_DROGAS AS (
    ...
    WHERE
        ...
        AND ENV.natureza_ocorrencia_codigo IN ('I04028', 'I04033')
        AND OCO.codigo_municipio IN (310670 , 310810 , 310900 , 311860 , 312060 , 312410 , 312600 , 312980 , 313010 , 313220 , 313665 , 314015 , 314070 , 315040 , 315460 , 315530 , 316292 , 316553)
)
SELECT
    H.nome_autor,
    ...
    H.data_hora_fato,
    H.nome_municipio,
    COUNT(D.data_hora_fato) AS num_passagens_drogas,
    MAX(D.data_hora_fato) AS ultima_data_prisao_drogas
FROM AUTORES_HOMICIDIOS H
LEFT JOIN PASSAGENS_DROGAS D
