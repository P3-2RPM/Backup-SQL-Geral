...
CHAMADA.descricao_evento 




FROM db_bisp_cad_reporting.vw_chamada_evento CHAMADA

WHERE 

CHAMADA.id_evento IN (2205, 2666) THEN 'SIM'
    ELSE 'NAO';
...
WITH Registro AS (
    SELECT 
        ...
        OCO.data_hora_fato AS data_registro, 
        OCO.numero_ocorrencia 
    FROM db_bisp_reds_reporting.tb_ocorrencia OCO 
    ...
    AND YEAR(OCO.data_hora_fato) >= 2021 
),
...
Atendimento AS (
    SELECT 
        ...
        ENV.data_nascimento, 
        OCO.codigo_municipio, 
        OCO.numero_ocorrencia AS numero_ocorrencia_atendimento, 
        OCO.data_hora_fato AS data_atendimento, 
        YEAR(OCO.data_hora_fato) AS ano_atendimento, 
        MONTH(OCO.data_hora_fato) AS mes_atendimento,
        ROW_NUMBER() OVER (
    ...
    INNER JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV 
        ON OCO.numero_ocorrencia = ENV.numero_ocorrencia 
    WHERE 
        OCO.natureza_codigo IN ('A20002', 'A20003', 'A20005', 'U33025') 
        AND YEAR(OCO.data_hora_fato) >= 2021 
        AND ENV.codigo_sexo = 'F'
),
...
RegistroUnico AS (
    SELECT 
        ...
        A.ano_atendimento,
        A.mes_atendimento,
        A.numero_ocorrencia_atendimento,
        ROW_NUMBER() OVER (
            PARTITION BY R.nome_completo_envolvido, R.data_nascimento
            ORDER BY R.data_registro DESC
        ) AS row_num
    FROM Registro R
    JOIN PrimeiroAtendimento A
        ON R.nome_completo_envolvido = A.nome_completo_envolvido
        AND R.data_nascimento = A.data_nascimento
        AND R.codigo_municipio = A.codigo_municipio
        AND R.data_registro < A.data_atendimento
)

SELECT DISTINCT
    I.nome_completo_envolvido, 
    DATEDIFF(I.data_atendimento, I.data_registro) AS dias_atendimento_seg_resposta, 
    I.ano_atendimento, 
    I.mes_atendimento, 
    I.numero_ocorrencia_atendimento 
FROM RegistroUnico I
WHERE I.row_num = 1
ORDER BY 
    I.ano_atendimento, 
    I.mes_atendimento, 
...
WITH Inclusao AS (
    SELECT 
        ...
        ENV.data_nascimento, 
        OCO.codigo_municipio, 
        OCO.data_hora_fato AS data_Inclusao, 
        YEAR(OCO.data_hora_fato) AS ano_Inclusao, 
        OCO.numero_ocorrencia 
    FROM db_bisp_reds_reporting.tb_ocorrencia OCO 
    INNER JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV 
        ON OCO.numero_ocorrencia = ENV.numero_ocorrencia 
    WHERE 
        OCO.natureza_codigo = 'A20003' 
...
Encerramento AS (
    SELECT 
        ...
        ENV.data_nascimento, 
        OCO.codigo_municipio, 
        OCO.numero_ocorrencia AS numero_ocorrencia_Encerramento, 
        OCO.data_hora_fato AS data_Encerramento, 
        YEAR(OCO.data_hora_fato) AS ano_Encerramento, 
        MONTH(OCO.data_hora_fato) AS mes_Encerramento,
        ROW_NUMBER() OVER (PARTITION BY ENV.nome_completo_envolvido, ENV.data_nascimento ORDER BY OCO.data_hora_fato) AS rn
    FROM db_bisp_reds_reporting.tb_ocorrencia OCO 
    INNER JOIN db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV 
        ON OCO.numero_ocorrencia = ENV.numero_ocorrencia 
    WHERE 
        OCO.natureza_codigo IN ('A20014', 'A20020', 'A20024') 
        AND YEAR(OCO.data_hora_fato) >= 2023 
        AND ENV.codigo_sexo = 'F'
),
InclusaoUnica AS (
    SELECT 
        ...
        I.data_Inclusao,
        I.numero_ocorrencia, 
        ROW_NUMBER() OVER (PARTITION BY I.nome_completo_envolvido, I.data_nascimento ORDER BY I.data_Inclusao) AS row_num 
    FROM Inclusao I 
)
SELECT DISTINCT
    I.nome_completo_envolvido, 
    DATEDIFF(E.data_Encerramento, I.data_Inclusao) AS dias_duracao_protocolo, 
    E.ano_Encerramento, 
    E.mes_Encerramento, 
    E.numero_ocorrencia_Encerramento 
FROM InclusaoUnica I 
JOIN Encerramento E 
    ON I.nome_completo_envolvido = E.nome_completo_envolvido 
    AND I.data_nascimento = E.data_nascimento 
    AND I.codigo_municipio = E.codigo_municipio 
WHERE 
    I.row_num = 1 -- Apenas o primeiro Inclusao do envolvido
    AND E.rn = 1  -- Apenas o primeiro Encerramento para esse envolvido
    AND I.data_Inclusao < E.data_Encerramento -- Garantir que a Inclusao seja antes do Encerramento
ORDER BY 
    E.ano_Encerramento, 
    E.mes_Encerramento, 
    I.nome_completo_envolvido;
     ...
     -- Subindicador de Reincidência. Total de reincidência de vítimas (independente de ser com o mesmo autor)
 ...
 WITH Reincidencias AS (
    SELECT
        ...
        OCO.data_hora_fato,
        YEAR(OCO.data_hora_fato) AS ano_fato, 
        COUNT(*) OVER (PARTITION BY ENV.nome_completo_envolvido, ENV.data_nascimento, YEAR(OCO.data_hora_fato)) AS num_ocorrencias,
        ROW_NUMBER() OVER (PARTITION BY ENV.nome_completo_envolvido, ENV.data_nascimento, YEAR(OCO.data_hora_fato) ORDER BY OCO.data_hora_fato DESC) AS rn
    FROM
        db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV
    JOIN
        db_bisp_reds_reporting.tb_ocorrencia OCO ON ENV.numero_ocorrencia = OCO.numero_ocorrencia
    WHERE
        ind_estado = 'F'
        AND YEAR(OCO.data_hora_fato) >= 2023
        AND ENV.id_envolvimento IN (25, 26, 27, 28, 32, 872, 1097)
        AND ENV.codigo_sexo = 'F'
        AND (
            OCO.natureza_codigo = 'U33004'
            OR OCO.natureza_secundaria1_codigo = 'U33004'
...
WHERE 
    ...
    AND ENV.codigo_sexo = 'F'
    AND ENV.condicao_fisica_descricao = 'FATAL'
    AND ENV.id_relacao_vitima_autor IN (3,4,5,9,15,16,18,22) -- RELAÇÃO VITIMA AUTOR CONJUGALIDADE (HOMEM/MULHER)
    AND (
    	OCO.natureza_codigo IN ('C01157','B01121','B01129','B02001')
		OR OCO.natureza_secundaria1_codigo IN ('C01157','B01121','B01129','B02001')
        OR OCO.natureza_secundaria2_codigo IN ('C01157','B01121','B01129','B02001')
        OR OCO.natureza_secundaria3_codigo IN ('C01157','B01121','B01129','B02001')
    )
...
SELECT 
    V.nome_vitima,
    V.data_nascimento_vitima,
    V.numero_ocorrencia,
    V.data_hora_fato,
    V.natureza_codigo,
    V.natureza_secundaria1_codigo,
    V.natureza_secundaria2_codigo,
    V.natureza_secundaria3_codigo,
    C.data_visita
FROM VITIMAS V
JOIN CASOS_PRIMEIRA_VISITA C 
    ON V.nome_vitima = C.nome_vitima
    AND V.data_nascimento_vitima = C.data_nascimento_vitima
    AND C.data_visita < V.data_hora_fato;




-- 2.4 Revitimização após o encerramento do Protocolo de Segunda Resposta (RPSR)
...
CASOS_ENCERRADOS AS (
    ...
    WHERE 
        ...
        AND OCO.ocorrencia_uf = 'MG'
        AND OCO.data_hora_fato >= '2021-01-01'
        AND OCO.natureza_codigo IN ('A20014', 'A20020', 'A20024')
),
...
AUTORES AS (
    ...
    JOIN db_bisp_reds_reporting.tb_ocorrencia OCO 
        ON ENV.numero_ocorrencia = OCO.numero_ocorrencia
    WHERE 
        ENV.id_envolvimento IN (1, 2, 35, 36)
)
...
SELECT 
    ...
    V.natureza_secundaria3_codigo,
    A.nome_autor,
    A.data_nascimento_autor
FROM VITIMAS V
JOIN CASOS_ENCERRADOS C 
    ON V.nome_vitima = C.nome_vitima
    AND V.data_nascimento_vitima = C.data_nascimento_vitima
    AND C.data_hora_fato < V.data_hora_fato
JOIN AUTORES A 
    ON V.numero_ocorrencia = A.numero_ocorrencia;
   
   
   -- Registros de Chamadas
SELECT 
EVE.numero_chamada,
EVE.data_hora_inclusao,
YEAR(EVE.data_hora_inclusao) AS ano_fato, 
MONTH(EVE.data_hora_inclusao) AS mes_num, -- Número do mês
COUNT (EVE.numero_chamada) AS contagem_chamada,
EVE.descricao_evento, 
CHAMADA.id_municipio as municipio_cod_chamada,
EMP.unidade_servico_nome, 
 CASE WHEN CHAMADA.id_municipio IN (310620) THEN '01 RPM'
   		WHEN CHAMADA.id_municipio IN (310670 , 310810 , 310900 , 311860 , 312060 , 312410 , 312600 , 312980 , 313010 , 313220 , 313665 , 314015 , 314070 , 315040 , 315460 , 315530 , 316292 , 316553) THEN '02 RPM'	
		WHEN CHAMADA.id_municipio IN (311000 , 311787 , 312170 , 313190 , 313460 , 313660 , 313760 , 314000 , 314480 , 314610 , 315390 , 315480 , 315670 , 315780 , 315900 , 316295 , 316830 , 317120) THEN '03 RPM'
	    WHEN CHAMADA.id_municipio IN (316720 , 314930 , 314110 , 314740 , 315360 , 310990 , 310500 , 311250 , 313570 , 313100 , 310320 , 312720 , 311890 , 312640 , 310960 , 315850) THEN '19 RPM'	
   	END AS RPM_2024,
CASE WHEN CHAMADA.id_municipio IN (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
		...
		WHEN CHAMADA.id_municipio IN (310163,310210,310290,310330,310560,310680,311630,312150,312500,312940,314570,314660,315030,315440,315730,315870,315940,316070,316620) THEN '09 BPM'
		...
		/* MUNIC PIOS COM MAIS DE 01 UEOP - 7 Munic pios*/
		WHEN CHAMADA.id_municipio =317020 AND (EMP.unidade_servico_nome LIKE '32 BPM%' or EMP.unidade_servico_nome LIKE '%/32 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '32 BPM'
		WHEN CHAMADA.id_municipio =317020 AND (EMP.unidade_servico_nome LIKE '17 BPM%' or EMP.unidade_servico_nome LIKE '%/17 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '17 BPM'
		WHEN CHAMADA.id_municipio =317010 AND (EMP.unidade_servico_nome LIKE '4 BPM%' or EMP.unidade_servico_nome LIKE '%/4 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '04 BPM'
		WHEN CHAMADA.id_municipio =317010 AND (EMP.unidade_servico_nome LIKE '67 BPM%' or EMP.unidade_servico_nome LIKE '%/67 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '67 BPM'
		WHEN CHAMADA.id_municipio =314330 AND (EMP.unidade_servico_nome LIKE '50 BPM%' or EMP.unidade_servico_nome LIKE '%/50 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '50 BPM'
		WHEN CHAMADA.id_municipio =314330 AND (EMP.unidade_servico_nome LIKE '10 BPM%' or EMP.unidade_servico_nome LIKE '%/10 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '10 BPM'
		WHEN CHAMADA.id_municipio =313670 AND (EMP.unidade_servico_nome LIKE '27 BPM%' or EMP.unidade_servico_nome LIKE '%/27 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '27 BPM'
		WHEN CHAMADA.id_municipio =313670 AND (EMP.unidade_servico_nome LIKE '2 BPM%' or EMP.unidade_servico_nome LIKE '%/2 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '02 BPM'
		WHEN CHAMADA.id_municipio =311860 AND (EMP.unidade_servico_nome LIKE '39 BPM%' or EMP.unidade_servico_nome LIKE '%/39 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '39 BPM'
		WHEN CHAMADA.id_municipio =311860 AND (EMP.unidade_servico_nome LIKE '18 BPM%' or EMP.unidade_servico_nome LIKE '%/18 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '18 BPM'
		WHEN CHAMADA.id_municipio =310670 AND (EMP.unidade_servico_nome LIKE '66 BPM%' or EMP.unidade_servico_nome LIKE '%/66 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '66 BPM'
		WHEN CHAMADA.id_municipio =310670 AND (EMP.unidade_servico_nome LIKE '33 BPM%' or EMP.unidade_servico_nome LIKE '%/33 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '33 BPM'
		WHEN CHAMADA.id_municipio =310620 AND (EMP.unidade_servico_nome LIKE '1 BPM%' or EMP.unidade_servico_nome LIKE '%/1 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '01 BPM'
		WHEN CHAMADA.id_municipio =310620 AND (EMP.unidade_servico_nome LIKE '5 BPM%' or EMP.unidade_servico_nome LIKE '%/5 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%') THEN '05 BPM'
		WHEN CHAMADA.id_municipio =310620 AND (EMP.unidade_servico_nome LIKE '13 BPM%' or EMP.unidade_servico_nome LIKE '%/13 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%') THEN '13 BPM'
		WHEN CHAMADA.id_municipio =310620 AND (EMP.unidade_servico_nome LIKE '16 BPM%' or EMP.unidade_servico_nome LIKE '%/16 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%') THEN '16 BPM'
		WHEN CHAMADA.id_municipio =310620 AND (EMP.unidade_servico_nome LIKE '22 BPM%' or EMP.unidade_servico_nome LIKE '%/22 BPM%' ) AND (EMP.unidade_servico_nome not LIKE '%TM%') THEN '22 BPM'
		WHEN CHAMADA.id_municipio =310620 AND (EMP.unidade_servico_nome LIKE '41 BPM%' or EMP.unidade_servico_nome LIKE '%/41 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '41 BPM'
		WHEN CHAMADA.id_municipio =310620 AND (EMP.unidade_servico_nome LIKE '49 BPM%' or EMP.unidade_servico_nome LIKE '%/49 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '49 BPM'
		WHEN CHAMADA.id_municipio =310620 AND (EMP.unidade_servico_nome LIKE '34 BPM%' or EMP.unidade_servico_nome LIKE '%/34 BPM%') AND (EMP.unidade_servico_nome not LIKE '%TM%')THEN '34 BPM'
		ELSE 'OUTROS'
	END AS UEOP_2024,
	CASE WHEN CHAMADA.id_municipio IN (
...
FROM db_bisp_cad_reporting.vw_chamada_evento EVE

LEFT JOIN db_bisp_cad_reporting.tb_chamada CHAMADA ON EVE.numero_chamada = CHAMADA.numero_chamada 

LEFT JOIN db_bisp_cad_reporting.tb_empenho EMP ON EMP.chamada_atendimento_id = EVE.id_chamada_atendimento 

WHERE 

YEAR (CHAMADA.data_hora_inclusao) >= 2023 
AND EVE.id_evento IN (2205, 2666)
