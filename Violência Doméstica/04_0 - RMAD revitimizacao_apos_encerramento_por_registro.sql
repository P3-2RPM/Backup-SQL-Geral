...
CASOS_ENCERRADOS AS (
    SELECT 
        ...
        OCO.natureza_secundaria2_codigo,
        OCO.natureza_secundaria3_codigo
--    ,ROW_NUMBER() OVER (PARTITION BY ENV.nome_completo_envolvido, ENV.data_nascimento ORDER BY OCO.data_hora_fato DESC) AS rn_geral_visitas
    FROM db_bisp_reds_reporting.tb_envolvido_ocorrencia ENV
    JOIN db_bisp_reds_reporting.tb_ocorrencia OCO 
        ON ENV.numero_ocorrencia = OCO.numero_ocorrencia
    WHERE 
...
SELECT 
    V.nome_vitima,
    1 as contagem_vitima,
    V.data_nascimento_vitima,
    V.numero_ocorrencia,
    V.data_hora_fato,
    year (V.data_hora_fato) as ano,
    V.natureza_codigo,
