...
ULTIMA_VISITA_ANTES_EVENTO AS (
    SELECT
        V.nome_vitima,
        V.data_nascimento_vitima,
        V.numero_ocorrencia AS numero_ocorrencia_evento,
        V.data_hora_fato AS data_hora_fato_evento,
        V.natureza_codigo AS natureza_codigo_evento,
        V.RPM_2024,
        V.UEOP_2024,
        V.nome_municipio,
        V.natureza_secundaria1_codigo AS natureza_secundaria1_codigo_evento,
        V.natureza_secundaria2_codigo AS natureza_secundaria2_codigo_evento,
        V.natureza_secundaria3_codigo AS natureza_secundaria3_codigo_evento,
        C.ocorrencia_visita,
        C.natureza_codigo,
        C.data_visita,
        ROW_NUMBER() OVER (PARTITION BY V.nome_vitima, V.data_nascimento_vitima, V.numero_ocorrencia ORDER BY C.data_visita DESC) AS rn
    FROM VITIMAS V
    JOIN CASOS_PRIMEIRA_VISITA C
        ON V.nome_vitima = C.nome_vitima
        AND V.data_nascimento_vitima = C.data_nascimento_vitima
    WHERE C.data_visita < V.data_hora_fato
)
SELECT
    U.nome_vitima,
    U.data_nascimento_vitima,
    U.numero_ocorrencia_evento,
    U.data_hora_fato_evento,
    U.natureza_codigo_evento,
    U.natureza_secundaria1_codigo_evento,
    U.natureza_secundaria2_codigo_evento,
    U.natureza_secundaria3_codigo_evento,
    U.RPM_2024,
    U.UEOP_2024,
    U.ocorrencia_visita,
    U.natureza_codigo,
    U.data_visita
FROM ULTIMA_VISITA_ANTES_EVENTO U
WHERE U.rn = 1;