WITH
OCOR_MV AS (
----- ESTA CTE BUSCA OCORRÊNCIAS DE MORTE VIOLENTA (GDO) SEM LETALIDADE POLICIAL -------
	SELECT 
	...
	WHERE 1=1 																		-- CONDIÇÃO SEMPRE VERDADEIRA PARA FACILITAR A ADIÇÃO DE OUTRAS CONDIÇÕES
		...
		AND env.natureza_ocorrencia_codigo in ('B01121','C01157','B02001','B01129','B01148','B01504') -- FILTRA O CÓDIGO DA NATUREZA DA OCORRÊNCIA: ROUBO, HOMICIDIO, LESÃO CORPORAL, FEMINICÍDIO, TORTURA
		AND env.digitador_id_orgao in (0,1) 										-- FILTRA O ID DO ORGÃO DIGITADOR, PM E PC
		AND YEAR (env.data_hora_fato) = :ANO 										-- PROMPT PARA FILTRAR O ANO DA DATA/HORA DO FATO
		AND env.numero_ocorrencia NOT IN
		(     
		SELECT DISTINCT                                                               
        LET.numero_ocorrencia                                           -- Seleciona o número da ocorrência da tabela de envolvido                                -- Seleciona o indicador se o militar estava em serviço
    	FROM 
		db_bisp_reds_reporting.tb_envolvido_ocorrencia AS LET 
    	WHERE 1=1                                                       -- Início das condições de filtro (1=1 é sempre verdadeiro)
        AND LET.natureza_ocorrencia_codigo IN('B01121','B01129')  		-- Filtra natureza específica (Homicídio, Lesão Corporal)
        AND LET.id_envolvimento IN (35,36,44)                          	-- Filtra apenas autores, co-autores e suspeitos
        AND LET.ind_militar_policial = 'M'          					-- Filtra apenas militares
        AND LET.ind_militar_policial_servico = 'S'  					-- Filtra apenas militares em serviço
        AND YEAR(LET.data_hora_fato) = :ANO                          	-- Filtra pelo ano informado no parâmetro
		)
),
PRESOS_A05 AS (
----- ESTA CTE BUSCA DADOS DE ENVOLVIDOS PRESOS DE AUTORES DE HOMICÍDIO CONSUMADO DEPOIS DO REGISTRO DO FATO -------
		SELECT 
		env.numero_ocorrencia, 														-- SELECIONA O NÚMERO DA OCORRÊNCIA
	...
	WHERE 1=1
        ...
        AND env.natureza_ocorrencia_codigo in ('B01121') 							-- FILTRA O CÓDIGO DA NATUREZA DO ENVOLVIDO: HOMICIDIO
		AND env.ind_consumado = 'S'        											-- FILTRA AUTORES DE CRIMES CONSUMADOS
        ...
        AND env.numero_ocorrencia IN (
        	SELECT 
        		alfacinco.numero_ocorrencia
        	FROM
        		db_bisp_reds_reporting.tb_ocorrencia alfacinco 						-- TABELA ENVOLVIDO
        	WHERE 1=1 
				AND alfacinco.natureza_codigo IN ('A05000') 						-- FILTRA O CÓDIGO DA NATUREZA DA OCORRÊNCIA: A05000
				AND alfacinco.digitador_sigla_orgao = 'PM' 							-- FILTRA ORGÃO DIGITADOR PM
				AND YEAR(alfacinco.data_hora_fato) = :ANO
		)
),
PRESOS_MV AS (
---- ESTA CTE BUSCA DADOS DE ENVOLVIDOS PRESOS POR MORTE VIOLENTA (GDO) -----
	...
	WHERE 1=1
        ...
        AND env.digitador_id_orgao = 0 												-- FILTRA O ID DO ORGÃO DIGITADOR PM 
        AND env.ind_militar_policial = 'M' 											-- FILTRA O QUE FOR DIFERENTE DE 'M' - INDICADOR DE POLICIAL MILITAR
        AND env.ind_militar_policial_servico = 'S' 									-- FILTRA O QUE FOR DIFERENTE DE 'S' - INDICADOR DE O POLICIAL MILITAR ESTÁ DE SERVIÇO
        AND env.numero_ocorrencia IN (
        	SELECT 
        	...
        	WHERE 1=1 
				...
				AND vit.natureza_ocorrencia_codigo IN ('B01121','C01157','B02001','B01129','B01148','B01504') -- FILTRA O CÓDIGO DA NATUREZA DA OCORRÊNCIA: ROUBO, HOMICIDIO, LESÃO CORPORAL, TORTURA
...
CASE WHEN oco.codigo_municipio in (310690,311590,311960,312130,312738,312850,314020,314950,315010,315540,315620,316290) THEN '02 BPM'
	...
	upper (oco.nome_bairro) AS nome_bairro, 												-- SELECIONA O NOME DO BAIRRO
	...
	COUNT (DISTINCT PALFA.numero_envolvido) as PresosALFA 									-- CONTA DE FORMA DISTINTA O NÚMERO DE ENVOLVIDOS DA CTE PA05
FROM 
	...
	LEFT JOIN PRESOS_A05 PALFA ON oco.numero_ocorrencia = PALFA.numero_ocorrencia 			-- JUNÇÃO COM A CTE PRESOS_A05, CHAVE NÚMERO DA OCORRÊNCIA
...
WHERE 1=1
	...
	AND oco.codigo_municipio IN (310670 , 310810 , 310900 , 311860 , 312060 , 312410 , 312600 , 312980 , 313010 , 313220 , 313665 , 314015 , 314070 , 315040 , 315460 , 315530 , 316292 , 316553) -- PARA RESGATAR APENAS OS DADOS DOS MUNICÍPIOS SOB SUA RESPONSABILIDADE, REMOVA O COMENTÁRIO E ADICIONE O CÓDIGO DE MUNICÍPIO. NO INÍCIO DO SCRIPT, É POSSÍVEL VERIFICAR ESSES CÓDIGOS, POR RPM E UEOP.
...
HAVING 																						-- FILTRA PARA QUE O CONTADOR PARA QUE RETORNE SEMPRE MAIOR QUE ZERO
    ...
    OR COUNT(DISTINCT PALFA.numero_envolvido) > 0 
ORDER BY 2,3,4;