-- IMV VERSÃO 3.2 30/04/2024
-- versão 3.2 adição de coluna de sexo, idade, latitude e longitude
WITH
LETALIDADE AS (
SELECT
...
WHERE 1=1
AND env.natureza_ocorrencia_codigo = 'B01121'
AND env.envolvimento_descricao in ('AUTOR','CO-AUTOR','SUSPEITO')
...
CAST(oco.codigo_municipio AS INTEGER) AS codigo_municipio,
oco.nome_municipio,
UPPER(oco.nome_bairro),
oco.numero_latitude,
...
LET.ind_militar_policial_servico AS LETALIDADE,
...
COUNT (
DISTINCT CASE WHEN (
env.natureza_ocorrencia_codigo IN ('C01157','B01121','B01129','B02001')
AND env.id_envolvimento IN (25,32,1097,26,27,28,872)
AND oco.ind_estado ='F' AND env.condicao_fisica_descricao = 'FATAL'
AND ocorrencia_uf ='MG' AND oco.digitador_sigla_orgao IN ('PM','PC','PP','SS'))
THEN ENV.numero_ocorrencia END
) AS 'IMV'
FROM
...
WHERE 1=1
AND oco.digitador_sigla_orgao IN ('PM','PC','PP','SS')
...
---------------------------------------------------------------------------
-- A LINHA ABAIXO É PARA TIRAR AS AUDITORIAS DE EXCLUSÃO:
---------------------------------------------------------------------------
AND env.numero_ocorrencia not in ('2024-002959697-001','2024-004057717-001','2024-005852151-001','2024-007943675-001','2024-015824319-001','2024-015827786-001','2024-014754259-002','2024-017297451-001', '2024-019873728-001','2024-022492319-001','2024-019226820-003','2024-023024798-001', '2024-024843620-001','2024-025063718-001')
---------------------------------------------------------------------------
