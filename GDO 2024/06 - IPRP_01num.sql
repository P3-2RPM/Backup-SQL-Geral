-- IPRP NUMERADOR VERSÃO 10/04/2024
WITH 
TODOS_PRESOS AS (
    SELECT 
        DISTINCT
...
SELECT 
    ...
    MONTH (ENV.data_hora_fato) AS mes_fato, 
    cast(ENV.data_nascimento as date) as 'data_nascimento', 
    ENV.natureza_ocorrencia_codigo COD_NATUREZA_ENVOLVIDO, 
    ENV.natureza_ocorrencia_descricao DESC_NATUREZA_ENVOLVIDO, 
    ENV.ind_consumado AS CONSUMADO_NATUREZA_ENVOLVIDO, 
    BO.digitador_sigla_orgao, 
    BO.natureza_codigo AS COD_NATUREZA_BO, 
    BO.natureza_descricao AS DESC_NATUREZA_BO, 
    BO.natureza_consumado AS CONSUMADO_NATUREZA_BO, 
...
INNER JOIN TODOS_PRESOS T ON (
    ...
    AND ENV.data_hora_fato > T.data_hora_fato
)
INNER JOIN db_bisp_reds_reporting.tb_ocorrencia BO ON BO.numero_ocorrencia = ENV.numero_ocorrencia
WHERE 1 = 1
	AND BO.digitador_sigla_orgao IN ('PM')
	AND BO.ind_estado IN ('F')
	AND BO.ocorrencia_uf  IN  ('MG')
