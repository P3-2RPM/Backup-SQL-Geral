-- ICCP VERSÃO 3.0 10/04/2024
WITH
OCO_VITIMAS AS (
	SELECT DISTINCT env.numero_ocorrencia, 1 as SINALIZA_VITIMA
	FROM db_bisp_reds_reporting.tb_envolvido_ocorrencia as env
	WHERE 1=1
		AND env.natureza_ocorrencia_codigo = 'C01155'
...
SELECT 
		...
		CAST (env.data_hora_fato AS DATE) as data_fato,
		HOUR (oco.data_hora_fato) as faixa_hora,
...
COUNT(DISTINCT CASE WHEN
    ...
    AND (
        (env.id_envolvimento IN (25,32,1097,26,27,28,872)) OR 					-- CÓDIGOS PARA VÍTIMAS: 25,26,27,28,32,872,1097
        (
            env.id_envolvimento IN (43) AND 									-- CÓDIGO PARA SOLICITANTE: 43
            OV.sinaliza_vitima IS NULL
            )
        )
    AND oco.ind_estado = 'F'
    AND ocorrencia_uf = 'MG'
    AND oco.digitador_sigla_orgao IN ('PM', 'PC')
	THEN env.numero_ocorrencia END) AS 'furto'
			,COUNT( CASE WHEN (
			env.natureza_ocorrencia_codigo IN ('C01157') 
			AND env.id_envolvimento IN (25,32,1097,26,27,28,872)  					-- CÓDIGOS PARA VÍTIMAS: 25,26,27,28,32,872,1097
			AND oco.ind_estado ='F' 
			AND ocorrencia_uf ='MG' AND oco.digitador_sigla_orgao  IN ('PM','PC'))  then ENV.numero_ocorrencia  END) AS 'roubo'
			,COUNT( CASE WHEN (
			env.natureza_ocorrencia_codigo IN ('C01158') 
			AND env.id_envolvimento IN (25,32,1097,26,27,28,872)  					-- CÓDIGOS PARA VÍTIMAS: 25,26,27,28,32,872,1097
			AND oco.ind_estado ='F' 
			AND ocorrencia_uf ='MG' AND oco.digitador_sigla_orgao  IN ('PM','PC'))  then ENV.numero_ocorrencia  END) AS 'extorsao'						
FROM 
...
LEFT JOIN OCO_VITIMAS OV
	ON oco.numero_ocorrencia = OV.numero_ocorrencia
WHERE	1=1
	AND 	oco.digitador_sigla_orgao IN  ('PM','PC')
	AND 	oco.ocorrencia_uf  IN  ('MG')
	AND 	env.natureza_ocorrencia_codigo IN ('C01155','C01157','C01158') 
	AND 	(env.id_envolvimento IN (25,32,1097,26,27,28,872) OR (  					-- CÓDIGOS PARA VÍTIMAS: 25,26,27,28,32,872,1097
			env.id_envolvimento IN (43) AND env.natureza_ocorrencia_codigo = 'C01155')) -- CÓDIGO PARA SOLICITANTE: 43
